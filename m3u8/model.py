...
class M3U8(object):
    ...
    def dumps(self):
        ...
        if self.discontinuity_sequence:
            output.append(
                "#EXT-X-DISCONTINUITY-SEQUENCE:{}".format(
...
class RenditionReport(BasePathMixin):
    ...
    def dumps(self):
        ...
        if self.last_part is not None:
            report.append("LAST-PART=" + number_to_string(self.last_part))
...
class Skip(object):
    ...
    def dumps(self):
        skip = []
        skip.append("SKIPPED-SEGMENTS=%s" % number_to_string(self.skipped_segments))
