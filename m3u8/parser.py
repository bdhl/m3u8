...
def _parse_extinf(line, data, state, lineno, strict):
    ...
    attrs = {}
    if len(chunks) == 2:
        ext_data, title = chunks
        duration, other_attrs = ext_data.split(" ", 1)
        if other_attrs:
            other_attrs = other_attrs.split(" ")
            attrs = dict(
                tuple(map(lambda c: c.strip('"'), v.split("="))) for v in other_attrs
            )
    elif len(chunks) == 1:
        if strict:
            raise ParseError(lineno, line)
...
def _parse_extgrp(line, data, state):
    ...
    state["segment"]["category"] = copy.copy(data["category"])
...
def _parse_stream_inf(line, data, state):
    ...
    atribute_parser = remove_quotes_parser(
        "codecs", "audio", "video", "subtitles", "closed_captions"
    )
